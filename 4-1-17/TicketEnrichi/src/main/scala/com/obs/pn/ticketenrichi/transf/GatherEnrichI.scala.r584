...
object GatherEnrichI {
  val logger: org.slf4j.Logger = LoggerFactory.getLogger(GatherEnrichI.getClass)
/**
 ...
 */
  ...
  def renameCategoriesIncidentCol(categoriesIncidents: DataFrame): DataFrame = {
    logger.debug("renameCategoriesIncidentCol")
    categoriesIncidents.withColumnRenamed("CATEGORIE", "category").withColumnRenamed("SOUS-CATEGORIE", "subcategory")
  }
  ...
  def dropColJoinResult(dataFrame: DataFrame): DataFrame = {
    logger.debug("dropColJoinResult")
    dataFrame.drop("cause_nrgtr").drop("responsabilite_nrgtr").select("*")
  }
