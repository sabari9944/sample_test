...
object IncidentologieFosav {
  val logger: org.slf4j.Logger = LoggerFactory.getLogger(IncidentologieFosav.getClass)
  /**
  ...
  */
  ...
  def rddToDF(fosav: DataFrame, replacedDf: RDD[Row]): DataFrame = {
    logger.debug("rddToDF")    
    fosav.sqlContext.createDataFrame(replacedDf, fosav.schema)
  }
  ...
  def renameColumns(fosav: DataFrame): DataFrame = {
    logger.debug("renameColumns")
    fosav.withColumnRenamed("pht_idtcom", "feuillet")
  }
