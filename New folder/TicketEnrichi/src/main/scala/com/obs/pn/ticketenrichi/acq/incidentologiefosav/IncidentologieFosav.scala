...
object IncidentologieFosav {
  ...
  def transform(fosav: DataFrame): DataFrame = {
    val ord = orderFosav(fosav)
    val repStr = replaceString(ord)
  ...
  /**
   * Order the records in incidentologie_FOSAV datframe by column "pht_idtcom"
   */

  def orderFosav(fosav: DataFrame): DataFrame = {
    val orderedFOSAV = fosav.orderBy("pht_idtcom")
    return orderedFOSAV
