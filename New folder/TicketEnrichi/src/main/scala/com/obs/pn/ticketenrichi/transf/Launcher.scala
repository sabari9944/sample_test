...
import com.obs.pn.acq.communesinseeutile.CommunesInseeUtile
import com.obs.pn.acq.correspondancecodeinseecodepostalutile.CorrespondanceCodeInseecodePostalUtile
import com.obs.pn.ticketenrichi.acq.incidentologiefosav.IncidentologieFosav
import com.obs.pn.acq.incidentologiewasac.IncidentologieWasac
...
object Launcher {
  ...
  def main(args: Array[String]) {
     ...
     */
    logger.info(" STARTING TICKET ENRICHI");
  ...
  def ticketenrichi() = {
    ...
    logger.info( "remove lkpRepetitif");
    var lkpRepetitif = Utils.sqlContext.emptyDataFrame
    
    logger.info( "remove ticketEnrich");
    var ticketEnrich = Utils.sqlContext.emptyDataFrame
    
    logger.info( "loading TIMESTAMP FOSAV");
    val fosav = Acquisition.loadTimestampFile("dev.fosav", Utils.runDate, false)
    
    logger.info( "loading TIMESTAMP WASAC => NO");
    //val wasac = Acquisition.loadTimestampFile( "dev.wasac", Utils.runDate, false);
    
    // ????????????? TO BE CLARRIFIED ?????????????????
    val loadInterimFosavWasac = Acquisition.loadFile( "dev.intermediateFosavWasac", false);
    
    logger.info( "loading TIMESTAMP ACORT");
    val acort = Acquisition.loadTimestampFile( "dev.acort", Utils.runDate, false);

    logger.info( "loading TIMESTAMP WASAC IAI");
    val wasacIai = Acquisition.loadTimestampFile("dev.wasac_iai", Utils.runDate, false)
    
    logger.info( "loading REFERENTIAL COMMUNES");
    val communes = Acquisition.loadFile("dev.communes", true)
    
    logger.info( "loading REFERENTIAL CORRESPONDANCE INSEE");
    val correspondence = Acquisition.loadFile("dev.correspondance_insee", true)

    logger.info( "loading REFERENTIAL RAP REFRENCE");
    val allRapReference = Acquisition.loadFile( "dev.rapReference",  true);
    
    logger.info( "loading REFERENTIAL INCIDENTS => NO");
    //val categoriesIncidents = CategoriesIncidents.loadFile()
    
    logger.info( "loading REFERENTIAL NRGTR");
    val nrgtrreferentiel = Acquisition.loadFile("dev.NRGTRreferentiel", true)
        
    logger.info( "loading REFERENTIAL LKP REPETITIF");
    if (Utils.prop.getString("dev.initialRun").equals("TRUE")) {
      lkpRepetitif = Acquisition.loadFile("dev.lkp_repetitifs", true).cache() 
    } else {
      lkpRepetitif = LKPRepetitif.lookReplicate()
    }
    
    logger.info( "loading LKP ACTIVATION");
    val lkpActivations = LKPActivations.loadFile()
    
    logger.info( "loading SOIPAD");
    val soipadIndicOceane = SoipadIndicOceane.loadFile()
    
    logger.info( "*** loadings ARE DONE ***");
    
      logger.info( "Stopping for now, some files are not loaded...");
...
//
//    val joinFosavAcort = this.joinFosavAcort(loadInterimFosavWasac, acort)
//    val acortTrans = FilterEnrichI.transformAcort(joinFosavAcort, acort)
...
//
//    val loadIntermediateAcort = Acquisition.loadFile("dev.filterResult", false) 
...
//    val inseeInterim = Acquisition.loadFile("dev.inseeResult", true) 
