...
object TransformEnrichI {
   ...
   */
  def replaceSTToSaint(s: String): String =
    {
      val result = s.replace("ST", "SAINT")
      result
    }
  ...
  def villeExtremiteATransform(fosavAcortAllRapRef: DataFrame): DataFrame = {
    val villeExtremiteAColumn = fosavAcortAllRapRef.select("num_ticket", "ville_extremite_A")

    val villeExtremiteAWithST = villeExtremiteAColumn.filter(villeExtremiteAColumn("ville_extremite_A").startsWith("ST "))
    val villeExtremiteAWithOutST = villeExtremiteAColumn.filter(!villeExtremiteAColumn("ville_extremite_A").startsWith("ST "))
    val villeExtremiteAWithSTTransf = villeExtremiteAWithST.map(x => { villaSchema(x(0).toString, replaceSTToSaint(x(1).toString)) }).toDF
    val villeExtremiteAOutput = villeExtremiteAWithSTTransf.unionAll(villeExtremiteAWithOutST).distinct()

    val villeExtremiteATransform = fosavAcortAllRapRef.as('a).join(villeExtremiteAOutput.as('b), $"a.num_ticket" === $"b.num_ticket", "left_outer").drop(fosavAcortAllRapRef("ville_extremite_A")).drop(villeExtremiteAOutput("num_ticket"))
