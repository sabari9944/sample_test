...
object SubGraph3 {
  ...
  def execute(dataframe1: DataFrame, dataFrame2: DataFrame) {
    ...
    val communes = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", Acquisition.SEMICOLON).option("inferSchema", "true").option("header", "true").load(Acquisition.getFilename("dev.communes"))
    logger.info("DELIMITER LOADED")
    /** Reading all the field from correspondance-code-insee-code-postal_utile */
    val correspondance = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", Acquisition.SEMICOLON).option("inferSchema", "true").option("header", "true").load(Acquisition.getFilename("dev.correspondance_insee"))
    ...
    //sqlContext.read.format("com.databricks.spark.csv").option("delimiter", Acquisition.SEMICOLON).option("inferSchema", "true").option("header", "true").option("parserLib", "univocity").load(Acquisition.getFilename("dev.output_sub_graph_2_pn_parc_marine2"))
    ...
    val dfIncidentologieWasacIai = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", Acquisition.SEMICOLON).option("parserLib", "univocity").option("header", "true").option("inferSchema", "true").load(Acquisition.getTimestampFilename("dev.wasac_iai", Utils.runDate))
    ...
    // no timestamp ? parcMarine.write.mode(SaveMode.Overwrite).parquet(Acquisition.getTimestampFilename("dev.output_sub_graph_3_pn_parc_marine2", Utils.runDate))

    logger.info("Writing parquet file for hive...")
    parcMarine.write.mode(SaveMode.Overwrite).parquet(Acquisition.getFilename("dev.output_sub_graph_3_pn_parc_marine2"))
    logger.info("parquet file written")

    logger.info("Writing .CSV file ...")
    parcMarine.write.format("com.databricks.spark.csv").save(Acquisition.getFilename("dev.test_csv_output"))
    logger.info("CSV file written")
    logger.info(" Lines in DataFrame " + parcMarine.count().toString())
